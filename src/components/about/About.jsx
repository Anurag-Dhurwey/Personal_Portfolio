...
const About = () => {
  ...
  const handleWheel=(e)=>{
    ...
    setProgress((pre)=>{return pre+wheelProgress})
    animate()
      ...
      <div className="relative h-[100vh] w-[100%] flex justify-center items-center flex-col overflow-hidden pointer-events-none">
        {img.map((item, i) => {
          ...
          return (
            <div
              key={i}
              style={{translate:`${x}% ${y}%`, rotate: `${rotate}deg`,zIndex ,opacity:`${zIndex/img.length*3-2}`,transitionDuration:`1s`}}
              className={`${i==activeElement?'h-[var(--ac-height)] w-[var(--ac-width)]':'h-[var(--height)] w-[var(--width)]'}  absolute rounded-xl overflow-hidden shadow-[0_10px_50px_10px_rgba(0,0,0,.5)] origin-[0%_100%]  pointer-events-none select-none `}
            >
